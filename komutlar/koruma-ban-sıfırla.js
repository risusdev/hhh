  ...
  if (kontrol == "agayokaga") {
    ...
    if (!kanal) {
      const embed = new Discord.MessageEmbed()
        ...
        .setDescription(`<a:twitchbit:793899916614828062> Ban koruma sistemi zaten ayarlanmamış!`);
    ...
    const embed = new Discord.MessageEmbed()
      .setColor(0x36393F)
      .setFooter(client.user.username, client.user.avatarURL())
      .setDescription(`<a:twitchbit:793899916614828062> Ban koruma sistemi sıfırlandı!`);
    message.channel.send(embed);
    return;
  } else {
    let kanal = await db.fetch(`bank_${message.guild.id}`)
